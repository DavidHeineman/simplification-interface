...
{
    // Variables generated by python script
    var paragraphs = [paragraphs_here];
    var candidates_list = [candidates_list_here];
    var pg_set = 0;
...
function generateView(i, j) {
    ...
    document.getElementById('total').innerHTML = paragraphs.length;
    document.getElementById('pg').innerHTML = paragraphs[i].substring(0, candidates_list[i][j][0]);
    var highlight = document.createElement("span");
    highlight.classList.add('highlighed-sent');
    highlight.innerHTML = " ".concat(paragraphs[i].substring(candidates_list[i][j][0], candidates_list[i][j][0] + candidates_list[i][j][2]).concat(" "));
    var word = document.createElement("span");
    word.classList.add('highlighted-word');
    word.innerHTML = " ".concat(paragraphs[i].substring(candidates_list[i][j][0] + candidates_list[i][j][2], candidates_list[i][j][0] + candidates_list[i][j][3]));
    highlight.appendChild(word);
    highlight.append(" ".concat(paragraphs[i].substring(candidates_list[i][j][0] + candidates_list[i][j][3], candidates_list[i][j][1]).concat(" ")));
    document.getElementById('pg').appendChild(highlight);
    document.getElementById('pg').append(paragraphs[i].substring(candidates_list[i][j][1]));
    document.getElementById('word-list').innerHTML = '';

    // Generate Sortable Word List
    for (var k = 0; k < candidates_list[i][j][4].length; k++) {
        var word = candidates_list[i][j][4][k];
